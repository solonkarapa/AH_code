...
# MELD
roc_meld_new <- roc(test.meld$D90_surv, test.meld$meld.surv.updated)
roc_meld_old <- roc(stph.meld$D90_surv, stph.meld$MELD.surv)
roc.test(roc_meld_old, roc_meld_new) # p-value = 0.864

# CLIF-C ACLF
roc_clif_new <- roc(test.clif$D90_surv, test.clif$clif.surv.updated)
roc_clif_old <- roc(stph.clif$D90_surv, stph.clif$CLIF.surv)
roc.test(roc_clif_new, roc_clif_old) # p-value = 0.992

# Lille
roc_lille_new <- roc(test.lille$D90_surv, test.lille$lille.surv.updated)
roc_lille_old <- roc(stph.lille$D90_surv, stph.lille$Lille.surv)
roc.test(roc_lille_new, roc_lille_old) # p-value = 0.9996
...
stph.cnew$meld.mort.new <- 1 - stph.cnew$meld.surv.updated
stph.cnew$lille.mort.new <- 1 - stph.cnew$lille.surv.updated
...
boot.diff.meld <- boot(data=stph.cnew, statistic = nb_diff,
...
boot.diff.clif <- boot(data=stph.cnew, statistic = nb_diff,
...
boot.diff.lille <- boot(data=stph.cnew, statistic = nb_diff,
...
plot(nb_data_lille$threshold, nb_data_lille$none, type = "l", lwd = 2, xlab = "Threshold probability", ylab = "Net benefit", ylim = c(-0.05, 0.25))
